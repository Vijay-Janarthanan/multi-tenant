...
from .utils import sendEmail, createTempassword, encodeJWTaccessToken, encodeJWTRefreshToken, decodeJWT
...
def signin(request):
    try:
        ...
        mail_str = f'''<html><head><body><h1>you have successfully logged in to Multi-Tenant</h1>
            ...
            # Send invite email
        sendEmail(email,mail_str,"Recent login to Multi-Tenant")
        payload = {"email":user['email']}
        return Response({
            'refresh': str(encodeJWTRefreshToken(payload, time_now)),
            'access': str(encodeJWTaccessToken(payload, time_now)),
        })
    except Exception as e:
        return Response({
            "stat":"Not Ok",
...
def resetpassword(request):
    try:
        ...
        if not User.objects.filter(email=email).exists():
            ...
            # Send invite email
            sendEmail(email,mail_str,"Password Changed")
            return Response({
            "stat":"Ok",
...
def inviteMember(request):
    try:
        ...
        try:
            ...
            # Send invite email
            sendEmail(user_email,mail_str,"You are Invited!!!")
